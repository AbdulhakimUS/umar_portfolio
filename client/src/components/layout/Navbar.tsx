    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'backdrop-blur-md bg-navy/80 border-b border-gold/10' : ''}`}>
        ...
        <div className="hidden md:flex items-center gap-8">
          {links.map(l => (
            <button key={l} onClick={() => scrollTo(l.toLowerCase())}
              className="text-sm text-gray-400 hover:text-gold transition-colors">{l}</button>
          ))}
          {profile?.openToWork && (
            <span className="px-3 py-1 rounded-full bg-gold/20 text-gold text-xs border border-gold/30">Open to Work</span>
          )}
        </div>
      ...
      {open && (
        <div className="md:hidden bg-navy/95 backdrop-blur border-t border-gold/10 px-6 py-4 flex flex-col gap-4">
          {links.map(l => (
            <button key={l} onClick={() => scrollTo(l.toLowerCase())} className="text-gray-300 hover:text-gold text-left">{l}</button>
          ))}
        </div>
      )}
    </nav>
