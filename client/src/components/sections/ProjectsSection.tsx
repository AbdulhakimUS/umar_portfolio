          ...
          <ScrollReveal key={p.id} delay={0.1}>
            <div className={`grid md:grid-cols-2 gap-8 items-center ${i % 2 === 1 ? 'md:[direction:rtl]' : ''}`}>
              <div className={i % 2 === 1 ? '[direction:ltr]' : ''}>
                <div className="aspect-video rounded-xl bg-navy-light border border-white/5 overflow-hidden hover:border-gold/40 transition-all hover:scale-[1.01]">
              ...
              <div className={i % 2 === 1 ? '[direction:ltr]' : ''}>
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {(Array.isArray(p.tags) ? p.tags : []).map((t: string) => (
                    <span key={t} className="px-3 py-1 bg-gold/10 text-gold text-xs rounded-full border border-gold/20">{t}</span>
                  ))}
                </div>
